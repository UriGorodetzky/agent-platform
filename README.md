...
By default every agent is a mock, so the orchestration works out of the box. To
have a role do real work with the Claude Code CLI (costs tokens), opt in:

```bash
CLAUDE_ROLES=coding ./.venv/Scripts/python.exe -m uvicorn orchestrator.api:app
```
